  ...
  return (
    <Link href={`/newsletters/${id}`}>
      <div className="group flex items-center gap-4 px-4 py-3.5 rounded-xl border border-white/[0.04] hover:border-white/[0.08] hover:bg-white/[0.02] transition-all duration-200 cursor-pointer">
        ...
        <span className="font-mono text-[11px] text-white/50 shrink-0 hidden sm:inline">
          {articleCount} articles
        </span>
        <span className="font-mono text-[11px] text-white/50 shrink-0 text-right">
          {date}
        </span>
        <ChevronRight className="w-3.5 h-3.5 text-white/45 group-hover:text-white/65 transition-colors shrink-0" />
      </div>
    </Link>
  );
}

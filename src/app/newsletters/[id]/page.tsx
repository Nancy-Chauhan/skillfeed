  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#09090B]">
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-6 pt-24 pb-10 space-y-8">
        {/* Back button */}
        <Link href="/dashboard">
          <Button variant="ghost" className="text-white/45 hover:text-white/70 hover:bg-white/[0.04] -ml-2 cursor-pointer text-[13px]">
        ...
        <div className="space-y-1">
          <p className="font-mono text-[10px] text-white/40 tracking-[0.15em] uppercase mb-4">
            Featured for you
          </p>
                  ...
                  <div>
                    {article.url ? (
                      <a
                        ...
                        className="text-sm font-semibold text-white/70 hover:text-white transition-colors inline-flex items-center gap-1.5"
                      >
                        {article.title}
                        <ExternalLink className="w-3 h-3 text-white/35 shrink-0" />
                      </a>
                    ) : (
                      <p className="text-sm font-semibold text-white/70">{article.title}</p>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-[10px] text-emerald-400/60 bg-emerald-400/[0.08] px-2 py-0.5 rounded">
                      {LEVEL_LABELS[article.level as Level] ?? article.level}
                    </span>
                    {article.source_name && (
                      <span className="text-[11px] text-white/40">{article.source_name}</span>
                    )}
                  </div>
                  {article.summary && (
